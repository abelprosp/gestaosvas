...
export const POST = createApiHandler(async (req) => {
  ...
  try {
    ...
    console.error("[POST /api/contracts] Erro ao fazer parse do JSON:", error);
    throw new HttpError(400, "Corpo da requisição inválido ou malformado. Verifique o formato JSON.");
  }
  
  let payload;
  try {
    payload = createContractSchema.parse(body);
  } catch (error) {
    if (error instanceof z.ZodError) {
      ...
      console.error("[POST /api/contracts] Erro de validação:", error.errors);
