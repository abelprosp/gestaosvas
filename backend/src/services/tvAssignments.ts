...
import { mapTVSlotRow } from "../utils/mappers";
...
async function fetchAvailableSlot() {
  ...
  if (historyError && !isSchemaMissing(historyError as PostgrestError)) {
    // Se o histórico não existe ainda, retorna o primeiro slot disponível
  ...
  // Retorna o primeiro slot que nunca foi atribuído
  const availableSlot = allSlots.find((slot) => !usedSlotIds.has(slot.id));
